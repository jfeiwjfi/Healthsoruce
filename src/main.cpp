...
#if defined(NDEBUG)
# error "Healthsource cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Healthsource Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Healthsource: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Healthsource: find block wanted by given orphan block
...
{
    int64_t nSubsidy = 0.00000001 * COIN;
    //nSubsidy = (nCoinAge * (0.01 * COIN)) / 365;
...
// Healthsource: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Healthsource miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Healthsource: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Healthsource: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Healthsource: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Healthsource: track money supply and mint amount info
...
// Healthsource: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Healthsource: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Healthsource: compute stake entropy bit for stake modifier
    ...
    // Healthsource: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Healthsource: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Healthsource: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Healthsource: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Healthsource-loadblk");
